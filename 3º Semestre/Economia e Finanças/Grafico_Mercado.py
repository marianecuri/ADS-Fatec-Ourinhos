...
def grafico_demanda():
    ...
    def desloc_demanda(quantidade, preco, outros_produtos, renda):
        return demanda(quantidade + desloc_demanda_init) - 1 * preco + outros_produtos - 1 * preco + (2/35) * renda
...
botao1 = Button(axBotao1, 'Grafico de Demanda')

def botao_demanda(event):
    grafico_demanda()
botao1.on_clicked(botao_demanda)

# Criando e dando função ao botão de oferta
axBotao2 = plt.axes([0.25, .65, 0.5, 0.1])
botao2 = Button(axBotao2, 'Grafico de Oferta')

def botao_oferta(event):
    grafico_oferta()
botao2.on_clicked(botao_oferta)
