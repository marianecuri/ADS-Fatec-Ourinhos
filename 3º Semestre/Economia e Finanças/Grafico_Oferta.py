...
def update(val):
    ...
    stem_vertical2.set_xdata([desloc_oferta_init + desloc_oferta_ponto.get_xdata()[np.abs(desloc_oferta_ponto.get_ydata() - slider_preco.val).argmin()]] * 2)
    ...
    stem_horizontal2.set_xdata([0, desloc_oferta_init + desloc_oferta_ponto.get_xdata()[np.abs(desloc_oferta_ponto.get_ydata() - slider_preco.val).argmin()]])
...
stem_vertical2, = ax.plot([desloc_oferta_init + desloc_oferta_ponto.get_xdata()[np.abs(desloc_oferta_ponto.get_ydata() - preco_init).argmin()]] * 2, [0, preco_init], '--', color='grey')
stem_horizontal1, = ax.plot([0, quantidade[np.abs(oferta(quantidade) - preco_init).argmin()]], [preco_init] * 2, '--', color='grey')
stem_horizontal2, = ax.plot([0, desloc_oferta_init + desloc_oferta_ponto.get_xdata()[np.abs(desloc_oferta_ponto.get_ydata() - preco_init).argmin()]], [preco_init] * 2, '--', color='grey')
